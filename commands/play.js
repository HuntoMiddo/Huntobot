...
module.exports = {
    ...
    execute: async ({client, interaction}) => {
        ...
        {
            await interaction.reply("You must be in a voice channel to use this command. Bitch")
            return;
        }  
           ...
           if (!interaction.member.voice.channel)
        {
         ...
         else    if (interaction.options.getSubcommand()=== "playlist")
         {
            ...
            if (result.tracks.length === 0)
            {
                await interaction.reply("no playlist found bitch");
                return
            }
         ...
         else    if (interaction.options.getSubcommand()=== "search")
         {
            ...
            if (result.tracks.length === 0)
            {
                await interaction.reply("no results found bitch");
                return
            }

            const psong =result.tracks[0];
            await queue.addTracks(song);
